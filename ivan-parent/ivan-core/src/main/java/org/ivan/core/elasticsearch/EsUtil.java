...
public class EsUtil {
	 ...
	 * @return
	 */
	public static boolean addIndex(String indexName){
		Es es = null;
		try{
			es = pool.borrowObject();
			return	es.addIndex(indexName);
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			pool.returnObject(es);
	 ...
	 * @return
	 */
	public static String  getWithId(String indexName, String typeName, String docId){
		Es es = null;
		try{
			es = pool.borrowObject();
			return	es.getWithId(indexName, typeName, docId);
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			pool.returnObject(es);
