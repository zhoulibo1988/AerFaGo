...
var orderDataParam = {   //订单数据参数
		...
		"proType": "",			  		//1理财,2基金,3证券,4借贷,5保险;
		"taskType": "1",		      	//1 开户 2 购买 查询时候传参    默认查询：开户
		"userId": "",			  		//用户ID
		"curPage": 1,			  		//当前页面
		"pageData": 10           		//页面数据量
}
...
if( !!prePageName && prePageName == "platData" ){    //如果是从 推广下商的平台列表进来则要传入推广
	$(".location a.location_details1").attr("page_url","promoter/platformList");
}
$(function(){
	//如果是从平台数据
	var defaultDate = getParamValueByName("defaultDate");   //默认日期
	if( !!defaultDate ){
		$("#createTimeBegin,#createTimeEnd").val(defaultDate);
		orderDataParam.createTimeBegin = defaultDate;
		orderDataParam.createTimeEnd = defaultDate;
	}
	...
	$("body").on("click","#exclusiveExportData",function(){   //导出数据
		createFormAndSubmit("POST","/export/exportAppProfitDetailed.do",orderDataParam,[]);
	});
...
function queryAppendOrderData(flag){
	...
	if( flag == true ){
		...
		fillInTableData(sectionObjInfo.tbody,"/UserTaskExecution/getTaskByAppId.do",orderDataParam,sectionObjInfo.trModuleIdName,true,"","","",function(result){
			$("."+sectionType).data("backuParam",copyJsonToOther( orderDataParam ) );
			//初始化所属机构下拉框
			initAffiliation(result);
			$("."+sectionType).find(".list-count .totalNum").text(result.Data.pageList.totalData);
			$("."+sectionType).find(".list-count .totalMoney").text(result.Data.balanceSum);
		},"",sectionObjInfo.pagination,2)
	}else{
		...
		orderDataParam.userId = $("#keyWord").val();                                          //用户id
...
function getTabSectionRelatedArea(sectionType){
	...
	var paramArr = ["appId","companyId","createTimeBegin","createTimeEnd","proType","taskType","userId"];
...
function initAffiliation(result){
	var companyList = result.Data.companyList;    //机构集合
	$.each(companyList,function(index,company){
		var html = "<li class='combobox-item' companyId='"+ company.companyId +"'>"+ company.companyName +"</li>";
