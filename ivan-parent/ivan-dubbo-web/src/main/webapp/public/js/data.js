...
$(".surebtn2").on("click", function() {
	if($(".data1").val() != "" && $(".data2").val() != "") {
		...
		cdata.startTime = $(".data1").val()+$(".time1").val();
		cdata.endTime = $(".data2").val()+$(".time2").val();
	}
	cdata.channelId = $("#channelId").val();
	cdata.companyId = $("#companyId").val();	
	cdata.appId = $("#appId").val();
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(!!data && data.resultCode == 200) {
				data = data.Data;
				$(".index-1 .index-1-data").text(data.payAmount);
				$(".index-2 .index-1-data").text(data.payCount);
				$(".index-3 .index-1-data").text(data.refundAmount);
				$(".index-4 .index-1-data").text(data.refundCount);
