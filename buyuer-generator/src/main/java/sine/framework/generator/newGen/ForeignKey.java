...
import sine.framework.generator.util.ListHashtable;
...
public class ForeignKey implements java.io.Serializable {
	...
	protected TableInfo parentTable;
	protected String tableName;
	protected ListHashtable columns;
	protected ListHashtable parentColumns;

	public ForeignKey(TableInfo aTable, String tblName) {
		super();
		parentTable = aTable;
		tableName = tblName;
	...
	private void initRelationship() {
		firstRelation = "";
		secondRelation = "";
		TableInfo foreignTable = null;
		try {
			foreignTable = (TableInfo) TableFactory.getTableFactory().getTable(
					tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private boolean hasAllPrimaryKeys(List<ColumnInfo> pkeys, ListHashtable cols) {
		boolean hasAll = true;
	...
	private boolean primaryKeyHasColumn(String aColumn) {
		...
		List<String> PKcolumnsList = parentTable.getPrimaryKeyColumns();
		for (String colname : PKcolumnsList) {
	...
	public TableInfo getSqlTable() {
		TableInfo table = null;
		try {
			table = TableFactory.getTableFactory().getTable(tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return table;
	...
	public static class ReferenceKey implements java.io.Serializable {
		...
		public ReferenceKey(String schemaName, String tableName,
		...
		public static ReferenceKey fromString(String foreignKey) {
			...
			if (!foreignKey.trim().matches(".*\\w+\\(.*\\)")) {
				throw new IllegalArgumentException("Illegal foreignKey:["
						+ foreignKey
						+ "] ,example value: fk_table_name(fk_column) ");
			}
			String schemaName = foreignKey.substring(0,
					Math.max(foreignKey.lastIndexOf("."), 0));
			String tableSqlName = foreignKey.substring(
					Math.max(foreignKey.lastIndexOf(".") + 1, 0),
					foreignKey.indexOf("("));
			String columnSqlName = foreignKey.substring(
					foreignKey.indexOf("(") + 1, foreignKey.indexOf(")"));
